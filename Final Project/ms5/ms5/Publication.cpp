...
namespace sdds {
   ...
   std::ostream& Publication::write(std::ostream& os) const {
      ...
      //Set the Title to show, and manually add '\0' to avoid error.
   ...
   //std::ostream& Publication::display(std::ostream& ostr) const{
   //   ostr << type() << "\t" << m_libRef << "\t";
   //   ostr << m_shelfId << "\t" << m_title << "\t";
   //   ostr << m_membership << "\t" << m_date;
   //   return ostr;
   //}

   std::ostream& Publication::formDisplay(std::ostream& ostr) const{
      char tempTitle[SDDS_TITLE_WIDTH + 1];
      strncpy(tempTitle, m_title, SDDS_TITLE_WIDTH);
      tempTitle[SDDS_TITLE_WIDTH] = '\0';
